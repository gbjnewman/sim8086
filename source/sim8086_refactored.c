...
void instdw_modregrm_displo_disphi(FILE* asm_file, int byte, char* instruction);
...
void mod10ref(FILE* asm_file, int byte, int byte2, char* instruction, int sbit);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	for (int i = 0; i < argc; i++)
	{
		...
		if (strcmp(argv[i], "--printbinary") == 0 || strcmp(argv[i], "-p") == 0)
		{
			print_binary = 1;
			printf("print binary flag is set\n");
		}
	}
...
void instdw_modregrm_displo_disphi(FILE* asm_file, int byte, char* instruction)
{
	// set up buffer for the instruction strings to go into
	char intruction_string_buffer[1024] = "";
	...
	//create print binary buffer
	char print_binary_buffer[1024] = "";
	...
	// add instruction to instruction string buffer
	sprintf
		(
	...
	if ((byte2 >> 6) == 0b01)
	{
		...
		if (print_binary)
		{
			sprintf
				...
				byte,
				byte2,
				// int8_t get cast to 32bits so masking off first 16bits
				(byte3 & 0b1111111111111111)
				);
		}

		// check if D bit is 1
		if (dbit) // if D is 1 reg is destination
		{
			if (byte3) // if byte3 is not zero add it into printf
			{
				sprintf
					(
					intruction_string_buffer + strlen(intruction_string_buffer),
			...
			else // if byte3 is zero then don't add it into printf
			{
				sprintf
					(
					intruction_string_buffer + strlen(intruction_string_buffer),
	...
	if ((byte2 >> 6) == 0b10)
	{
		//mod10ref(asm_file, byte, byte2, instruction, sbit);
		// read in 3rd byte for displacement
		int byte3 = fgetc(asm_file);
	...
	// print binary out if necessary
	if (print_binary)
	{
		printf(print_binary_buffer);
	}

	// print out instruction from instructin buffer
	printf(intruction_string_buffer);
}
...
// mod10 refactor attempt, memory mode, 16bit displacement follows
void mod10ref(FILE* asm_file, int byte, int byte2, char* instruction, int sbit)
{
	//create print binary buffer
	char print_binary_buffer[256] = "";
...
// jump instructions switch table
void jump_instruction(FILE* asm_file, int byte)
{
	int8_t byte2 = fgetc(asm_file);
