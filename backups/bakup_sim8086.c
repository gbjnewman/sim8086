...
goto skip_sub_cmp;
		...
		// sub immediate to regmem //TODO: fix this(mov import)convert add to sub
		if ((byte >> 2) == 0b100000)
		{
			// get second instruction byte
			int byte2 = fgetc(asm_file);
...
end_sub:
		...
		// cmp regmem with reg to either FIXME: convert add to cmp
		if ((byte >> 2) == 0b001110)
		{
			// read in the secong byte for this instruction type
			int byte2 = fgetc(asm_file);
		...
		// cmp immediate to regmem //TODO: fix this(mov import)convert add to cmp
		if ((byte >> 2) == 0b100000)
		{
			// get second instruction byte
			int byte2 = fgetc(asm_file);
